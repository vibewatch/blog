...
3. Read the source article and build a presentation outline with one idea per slide.
...
7. For Mermaid diagrams, copy the source from the article and trim it for slide scale: short node labels, `<br/>` for line breaks, and total source under ~2400 characters.
8. Run `npm run slides:validate` and fix every failure before considering the deck complete.
9. Run `npm run build` to verify the Astro route compiles.
